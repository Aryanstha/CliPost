...
func (e *AutocompleteEditor) Edit(v *gocui.View, key gocui.Key, ch rune, mod gocui.Modifier) {
	if key != gocui.KeyEnter {
		e.CliPostEditor.Edit(v, key, ch, mod)
	}
	...
	if key == gocui.KeyEnter && e.isAutocompleting {
		...
		closeAutocomplete(e.CliPostEditor.g)
	...
	cx, cy = v.Cursor()
	sx, _ := v.Size()
	ox, oy, _, _, _ := e.CliPostEditor.g.ViewPosition(v.Name())
	...
	if len(completions) > 0 {
		...
		showAutocomplete(comps, x, y, maxWidth, maxHeight, e.CliPostEditor.g)
...
func (e *SearchEditor) Edit(v *gocui.View, key gocui.Key, ch rune, mod gocui.Modifier) {
	e.CliPostEditor.Edit(v, key, ch, mod)
	e.CliPostEditor.g.Update(func(g *gocui.Gui) error {
		e.CliPostEditor.app.PrintBody(g)
		return nil
	})
}
...
func help() {
	fmt.Println(`CliPost - Interactive cli tool for HTTP inspection

Usage: CliPost [-H|--header HEADER]... [-d|--data|--data-binary DATA] [-X|--request METHOD] [-t|--timeout MSECS] [URL]
...
Other command line options:
  ...
  -T, --tls MIN,MAX        Restrict allowed TLS versions (values: SSL3.0,TLS1.0,TLS1.1,TLS1.2)
                           Examples: CliPost -T TLS1.1        (TLS1.1 only)
                                     CliPost -T TLS1.0,TLS1.1 (from TLS1.0 up to TLS1.1)
