...
func (d *Duration) UnmarshalText(text []byte) error { // HL
	var err error
...
var DefaultConfig = Config{
	General: GeneralOptions{
		...
		StatusLine:             "[CliPost {{.Version}}]{{if .Duration}} [Response time: {{.Duration}}]{{end}} [Request no.: {{.RequestNumber}}/{{.HistorySize}}] [Search type: {{.SearchType}}]{{if .DisableRedirect}} [Redirects Restricted Mode {{.DisableRedirect}}]{{end}}",
		Timeout: Duration{
...
func GetDefaultConfigLocation() string {
	...
	case "linux":
		...
		if xdgConfigHome != "" {
			...
			configFolderLocation, _ = homedir.Expand("~/.config/CliPost/")
		}

	default:
		// On other platforms we just use $HOME/.wuzz
		configFolderLocation, _ = homedir.Expand("~/.CliPost/")
	}
